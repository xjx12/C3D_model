...
class VideoDataset(Dataset):
    def __init__(self, dataset_path, images_path, clip_len):
        self.dataset_path = dataset_path  # 获取数据集的地址
        self.split = images_path  # 训练集，测试集，验证集的名字
        self.clip_len = clip_len  # 生成数据的深度

        self.resize_height = 128
        self.resize_width = 171
        self.crop_size = 112

        # 获取对应训练集/验证集/测试集下的各种类别的行为动作
        # 每个行为动作下的视频已经被处理成单个图片数据
        # 将对应动作的数据的文件名作为标签保存到labels列表中，对应的动作数据集的路径保存到self.fnames列表中
        ...
        for label in sorted(os.listdir(folder)):
            for fname in os.listdir(os.path.join(folder, label)):
                ...
                labels.append(label)
        print('Number of {} video: {:d}'.format(images_path, len(self.fnames)))
    ...
    def __getitem__(self, index):
        ...
        buffer = self.crop(buffer, self.clip_len, self.crop_size)  # 在数据的深度、高度、宽度方向进行随机裁剪，将数据转化为(16,112,112,3)
        buffer = self.normalize(buffer)  # 对数据进行归一化处理
        buffer = self.to_tensor(buffer)  # 对维度进行转换
    ...
    def crop(self, buffer, clip_len, crop_size):
        ...
        # 利用切片在视频图片上进行提取，获得一个（clip_len,112,112,3）数据
